(() => {
  ...
  var normalized, ranges, strToRunes, whitespaceRunes, MAX_ASCII, CAPITAL_A_RUNE, CAPITAL_Z_RUNE, SMALL_A_RUNE, SMALL_Z_RUNE, NUMERAL_ZERO_RUNE, NUMERAL_NINE_RUNE, SCORE_MATCH, SCORE_GAP_START, SCORE_GAP_EXTENTION, BONUS_BOUNDARY, BONUS_NON_WORD, BONUS_CAMEL_123, BONUS_CONSECUTIVE, BONUS_FIRST_CHAR_MULTIPLIER, fuzzyMatchV2, fuzzyMatchV1, exactMatchNaive, SLAB_16_SIZE, SLAB_32_SIZE, slab, buildPatternForBasicMatch, isNode, defaultOpts, BaseFinder, syncDefaultOpts, SyncFinder, asyncDefaultOpts, createResultItemWithEmptyPos, Fzf;
  var init_fzf_es = __esm({
    "node_modules/fzf/dist/fzf.es.js"() {
      normalized = {
        216: "O",
        223: "s",
      ...
      strToRunes = (str) => str.split("").map((s) => s.codePointAt(0));
      whitespaceRunes = new Set(
      ...
      MAX_ASCII = "\x7F".codePointAt(0);
      CAPITAL_A_RUNE = "A".codePointAt(0);
      CAPITAL_Z_RUNE = "Z".codePointAt(0);
      SMALL_A_RUNE = "a".codePointAt(0);
      SMALL_Z_RUNE = "z".codePointAt(0);
      NUMERAL_ZERO_RUNE = "0".codePointAt(0);
      NUMERAL_NINE_RUNE = "9".codePointAt(0);
      SCORE_MATCH = 16;
      SCORE_GAP_START = -3;
      ...
      fuzzyMatchV2 = (caseSensitive, normalize, forward, input, pattern, withPos, slab2) => {
      ...
      fuzzyMatchV1 = (caseSensitive, normalize, forward, text, pattern, withPos, slab2) => {
        if (pattern.length === 0) {
      ...
      exactMatchNaive = (caseSensitive, normalize, forward, text, pattern, withPos, slab2) => {
        if (pattern.length === 0) {
          return [{ start: 0, end: 0, score: 0 }, null];
        }
        const lenRunes = text.length;
      ...
      SLAB_16_SIZE = 100 * 1024;
      SLAB_32_SIZE = 2048;
      slab = makeSlab(SLAB_16_SIZE, SLAB_32_SIZE);
      buildPatternForBasicMatch = (query, casing, normalize) => {
      ...
      isNode = typeof __require !== "undefined" && typeof window === "undefined";
      defaultOpts = {
        limit: Infinity,
        selector: (v) => v,
        casing: "smart-case",
      ...
      SyncFinder = class extends BaseFinder {
        constructor(list, ...optionsTuple) {
      ...
      createResultItemWithEmptyPos = (item) => ({
        item,
        start: -1,
        end: -1,
        score: 0,
